...
namespace PoeHUD
{
    public class Program
    {
        ...
        {
            var clients = Process.GetProcessesByName(Offsets.Regular.ExeName)
                .Select(p => Tuple.Create(p, Offsets.Regular)).ToList();
            clients.AddRange(Process.GetProcessesByName(Offsets.Steam.ExeName)
                .Select(p => Tuple.Create(p, Offsets.Steam)));
        ...
        private static int chooseSingleProcess(List<Tuple<Process, Offsets>> clients)
        {
            ...
            String o2 =
                $"No - process #{clients[1].Item1.Id}, started at {clients[1].Item1.StartTime.ToLongTimeString()}";
            ...
            return answer == DialogResult.Cancel ? -1 :
                answer == DialogResult.Yes ? 0 : 1;
        }
        ...
        public static void Main(string[] args)
        {
            AppDomain.CurrentDomain.UnhandledException += (sender, exceptionArgs) =>
            {
                ...
                File.AppendAllText("Error.log",
                    $"{DateTime.Now.ToString("g")} {errorText}\r\n{new string('-', 30)}\r\n");
            ...
            {
                MessageBox.Show(
                    "Path of Exile is not running or you started x32 POE (DirectX9) with x64 PoeHUD or vice verse.");
                return;
            }
            ...
            using (var memory = new Memory(offs, pid))
            {
                ...
                var overlay = new ExternalOverlay(memory);
